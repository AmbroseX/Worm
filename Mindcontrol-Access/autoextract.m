...
%only get ymal file once in one worm's analysis
 ...
 fprintf('\n%s\n',filename);
 if ~exist('wormname','var')
    wormname = input('Input the worm name \n','s');
 end
 answer = inputdlg({'starting frame number before DLPon'}, '', 1, {num2str(350)});
 frames_beforeDLPon = str2num(answer{1});
 if ~exist('mcd','var')
     mcd=Mcd_Frame;
...
%set(gca,'YTICK',[1:50:numfram]);
%y_tick=get(gca,'YTICK');
%set(gca,'YTICKLABEL',time(y_tick));
%set(gca,'YTICKLABEL',[istart:100:iend]);
...
disp(vedio_sequence);
clear DLPon_frame DLPoff_frame j1 j2
%This version contains the completed functions of analysing worm behavior
%parameter.
...
for cycle=1:cyclenum 
    ...
    t_w_curdata{vedio_sequence,1}=curvdata;
    t_w_amp{vedio_sequence,1}=body_amp;
%    t_w_angledata{vedio_sequence,1}=if_bias_Before_DLPon(angle_data_zerooffset1);
