...
c2 = curvdatafiltered(t_start:t_end,:) > 0;%plot points that are >0
...
for n=1:numlab
    c5 = (c4 == n);
    [y,x] = find(c5);
    % using shifted points to determine the fitted line is after peak or vally.
    yshift = 5;
    ...
    %         disp(mean(curvshift));
            ...
            % exclude points near head and tail for fitting
             %but the coordination of x,y is extercted form data points
             %that head and tail be cut.
...
%istart_speedtrack=input('which frame do you want to start tracking ');
%iend_speedtrack=input('and to which frame to end ');
%stagepos=trackShmutz(vidfile,istart_speedtrack,iend_speedtrack);
