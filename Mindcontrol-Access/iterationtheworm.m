...
iteration= inputdlg({'is the worm illuminated?','Start frame', 'frames every iteration','interval frames'}, '', 1);
illumination_status=str2num(iteration{1});
begin_frm=str2num(iteration{2});
iteration_length=str2num(iteration{3});
interval_frm=str2num(iteration{4});
cyclenum=floor((numframes_total-begin_frm-iteration_length)/iteration_length);
time_auto=cell(cyclenum,1);
iteration_info=zeros(cyclenum,2);
for cycle=1:cyclenum 
    close all
    istart = begin_frm+(cycle-1)*(iteration_length+interval_frm);
    iend = istart+iteration_length;
    iteration_info(cycle,1)=istart;
    iteration_info(cycle,2)=iend;
    spline_p = 0.0005;
    flip = 0;
    ...
    this_worm_curvdatafiltered{vedio_sequence,1}=curvdatafiltered;
    ...
    if exist('DLPoff_frame','var')
        this_worm_aligning(vedio_sequence,5)=DLPoff_frame;
        this_worm_aligning(vedio_sequence,6)=initiation;
        this_worm_aligning(vedio_sequence,7)=origin_y+radius_y;
    else
...
evalc([wormname,'_amp=this_worm_amp']);
evalc([wormname,'_curdata=this_worm_curdata']);
evalc([wormname,'_curvdatafiltered=this_worm_curvdatafiltered']);
