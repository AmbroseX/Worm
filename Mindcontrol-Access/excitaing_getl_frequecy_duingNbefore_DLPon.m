...
answer = inputdlg({'limit for minimal period','the anterior portion to be calculated'}, '', 1);
minimalperiod=str2double(answer{1});
anterior_portion=str2double(answer{2});
t_w=cell(cyclenum,4);
     ...
     for i=1:illustatus %each i donotes different illumination state
        ...
        if i==3
         ...
         for j=2:numpks
             ...
             if pklcs(j-k,2)-pklcs(j-k-1,2)<minimalperiod*fps % get rid of fulse peaks
                 pklcs(j-k,:)=[];
                 k=k+1;
             end
         end
         k=0;
         for j=2:numpksI
             if pklcsI(j-k,2)-pklcsI(j-k-1,2)<minimalperiod*fps % get rid of fulse peaks
                 pklcsI(j-k,:)=0;
                 k=k+1;
             end
         end
