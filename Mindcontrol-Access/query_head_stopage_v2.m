...
answers2 = inputdlg({'bodyFraction_thrsh','min_y_thrshd','max_abs_y_distance'},'',1,{num2str(10),num2str(10),num2str(300)});
trail = str2num(answers{1});
region = str2num(answers{2});
flip_se = str2num(answers{3});
bodyFraction_thrsh = str2num(answers2{1});
min_y_thrshd = str2num(answers2{2});
max_abs_y_distance = str2num(answers2{3});
...
%% a curve that starts and ends from left_bound smaller than 6 and last longer than 5
     ...
     [y,x] = find(c5);
     if find(x == 1)
     [Ydistance,Stopage] = calculate_mean_y_distance_N_stopage(c5,bodyFraction_thrsh,min_y_thrshd,max_abs_y_distance);
     if Stopage
         if Ydistance < 150
             x_max(j) = Stopage;
         end
