...
answer = inputdlg('before (1) or during DLPon (2)','',1);
timing_selection = str2num(answer{1});
for sequence_extracting = 1:length(t_w_curvdatafiltered)
if timing_selection == 1
t1 = 1;
t2 = find(time_auto{sequence_extracting,1} == 0);
elseif timing_selection == 2
t1 = find(time_auto{sequence_extracting,1} == 0);
t2 = t_w_illu_time(sequence_extracting,2)-t_w_illu_time(sequence_extracting,1)+t1;
end

t_start = t1;
t_end = t2;

fps=(t_end-t_start)/(time_auto{sequence_extracting,1}(t_end,1)-time_auto{sequence_extracting,1}(t_start,1));
...
for n=1:numlab
    ...
    for jj=1:length(x)
        curvshift(jj) = t_w_curdata{sequence_extracting,1}(yshifted(jj), x(jj));
    end
    %         
    %         disp(n);
    %         disp('mean(c2shift)');
