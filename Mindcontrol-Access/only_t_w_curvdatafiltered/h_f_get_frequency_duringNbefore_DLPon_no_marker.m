...
answer = inputdlg({'limit for minimal period','and the maxmium interval','the anterior portion to be calculated','worm name'}, '', 1);
minimalperiod=str2double(answer{1});
anterior_portion=str2double(answer{3});
maxperiod=str2double(answer{2});
t_w_curvdatafiltered = evalin('base',[answer{4},'_curvdatafiltered']);
%t_w=cell(cyclenum,4);
...
for sequence_extracting_index=1:length(t_w_curvdatafiltered)
    
    shortafterflag=0;
    t1 = 1;
    t2 = size(t_w_curvdatafiltered{sequence_extracting_index},1);
    t1b=t1-t1+1;
    t2b=t2;
    t1d=t1;
    t2d=t2;
    ...
    for n=1:numlab
        ...
        for jj=1:length(x)
            curvshift(jj) = t_w_curvdatafiltered{sequence_extracting_index,1}(yshifted(jj), x(jj));
        end
    ...
    %whole body length
     ...
     angle_data_selected{1,1} = unwrap(unwrap(t_w_curvdatafiltered{sequence_extracting_index,1}(t1b:t2b,:)),[],2); 
     angle_data_selected{2,1} = unwrap(unwrap(t_w_curvdatafiltered{sequence_extracting_index,1}(t1d:t2d,:)),[],2);
     angle_data_selected{3,1} = unwrap(unwrap(t_w_curvdatafiltered{sequence_extracting_index,1}(t1d:t2d,:)),[],2);
     ...
     for i=1:illustatus % i == 1, before DLPon; i == 2, during DLPon; i == 3, 
        ...
        if i==3
           angle_smthp20{i}=smooth(smooth(smooth(smooth(angle_data_zerooffset1{i,1}(:,90),15,'sgolay'),15),15),15);
        else
