...
switch origin_y
    case 10
        ...
        segdata_head{sequence_1,1}=segmented_curv((j1-100):end,:);
    case 50
        %segdata_medial=cell(20,1);
        segdata_medial{sequence_1,1}=segmented_curv((j1-100):end,:);
    case 80
        %segdata_tail=cell(20,1);
        segdata_tail{sequence_1,1}=segmented_curv((j1-100):end,:);
end
%}
...
body_cur2=zeros(numframes-j1+1,99);
...
for bodyseg=2:99
    ...
    origin=bodyseg;
    radius=1;
    % amplitude before illminated
    for j=1:j1
        body_cur1(j,bodyseg)=mean(curvdatafiltered(j,origin-radius:origin+radius));
    end
    
    % amplitude during illminater
    for j=j1:numframes
        body_cur2(j,bodyseg)=mean(curvdatafiltered(j,origin-radius:origin+radius));
    end
    % amplitude after illminater
    %for j=j2:numframes
        %body_cur3(j,bodyseg)=mean(curvdatafiltered(j,origin-radius:origin+radius));
    end
    ...
    for bodyseg=1:99
        ...
        body_amp2(1,bodyseg)=std(body_cur2(:,bodyseg))*2;
        %body_amp3(1,bodyseg)=std(body_cur3(:,bodyseg))*2;
...
plot(1:99,body_amp1,'k',1:99,body_amp2,'g')
...
switch origin_y
    case 10
        %segdata_head=cell(20,1);
        amp_head{sequence_1,1}=body_amp;
    case 20
        %segdata_head=cell(20,1);
        amp_head{sequence_1,1}=body_amp;
    case 50
        %segdata_medial=cell(20,1);
        amp_medial{sequence_1,1}=body_amp;
    case 80
        %segdata_tail=cell(20,1);
        amp_tail{sequence_1,1}=body_amp;
end
