...
icasig = icasig';
%get kymograph , plot
[Kymo_curvdata_Tosif, Kymo_theta_Tosif] = Get_Kymograph(mixmat, Kymoratio_Tosif, eigenvectors, K_delay, K_mode, embedded_dimension);
len = floor(size(icasig, 1) / 2);
figure
hold on
title('ICA phase plot by Tosif')
subplot(1, 2, 1); plot(icasig(end - len:end, 2), icasig(end - len:end, 3));
xlabel('ICA_2')
ylabel('ICA_3')
subplot(1, 2, 2); plot(icasig(end - len:end, 6), icasig(end - len:end, 7));
xlabel('ICA_6')
ylabel('ICA_7')
hold off
...
xlabel('Z_2')
ylabel('Z_5')
subplot(1, 2, 2); plot(Zica(6, end - len:end), Zica(7, end - len:end));
xlabel('Z_6')
ylabel('Z_7')
hold off
